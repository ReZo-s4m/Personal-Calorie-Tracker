...
export function isAiOnline(status?: AiStatus | null) {
  return Boolean(status?.chatAvailable || status?.extractAvailable || status?.available);
}

export function isPhotoExtractAvailable(status?: AiStatus | null) {
  return Boolean(status?.extractAvailable ?? status?.available);
}
