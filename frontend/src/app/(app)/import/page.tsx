...
export default function ImportPage() {
  ...
  return (
    <div className={reviewing ? 'flex flex-col gap-4' : 'flex flex-col gap-5'}>
      <header className={reviewing ? 'page-masthead flex flex-wrap items-end justify-between gap-3' : 'page-masthead'}>
        <div>
          <p className="kicker">Upload</p>
          <h1 className="mt-1 text-[1.65rem] font-medium tracking-tight sm:text-3xl">Upload</h1>
          {!reviewing && (
            <p className="mt-1 text-sm text-muted">
              Upload a PDF, check the rows, then import when they look right.
            </p>
          )}
        </div>
        {reviewing && (
        ...
        <>
          ...
          {!preview ? (
            ...
            <section className="flex flex-col gap-4">
              ...
              <div className="sticky bottom-3 z-10 flex flex-wrap items-center justify-between gap-3 rounded-md border border-border bg-surface/95 px-5 py-4 shadow-[0_8px_24px_rgb(17_17_19/0.08)] backdrop-blur">
                <p className="text-sm text-muted">
