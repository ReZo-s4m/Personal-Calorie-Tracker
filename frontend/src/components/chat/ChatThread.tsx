  ...
  return (
    <div className="flex flex-col gap-5">
      {turns.map((turn) => (
        ...
        >
          {turn.role === 'assistant' && (
            <span className="mt-0.5 flex size-8 shrink-0 items-center justify-center rounded-sm bg-accent-soft">
            ...
            {turn.actions && turn.actions.length > 0 && (
              <ul className="flex w-full flex-col gap-1.5">
                {turn.actions.map((action, index) => (
                  <li
                    key={`${turn.id}-${index}`}
                    className="flex items-center justify-between gap-3 rounded-md border border-accent/20 bg-accent-soft px-3 py-2 text-xs text-accent"
                  >
                  ...
                  {turn.pendingAction.candidates.map((entry) => (
                    <li key={entry.entryId}>
                      <button
                        ...
                        className="w-full rounded-sm border border-border bg-surface-raised px-3 py-2 text-left text-xs hover:border-accent hover:bg-accent-soft"
                      >
      ...
      {isThinking && (
        <div className="flex items-center gap-3" role="status">
          <span className="flex size-8 shrink-0 items-center justify-center rounded-sm bg-accent-soft">
            <BrandMark size={18} withName={false} />
          </span>
...
function AssistantOrUserBody({ turn }: { turn: ThreadTurn }) {
  if (turn.role === 'user') {
    return (
      <div className="rounded-md bg-foreground px-4 py-3 text-sm whitespace-pre-wrap text-on-accent">
        {turn.content}
      </div>
    );
  }
  ...
  return (
    <div className="flex w-full flex-col gap-2 rounded-md border border-border bg-surface px-4 py-3 text-sm text-foreground">
      {parts.before && <p className="whitespace-pre-wrap">{parts.before}</p>}
      {parts.table && (
        <pre className="max-w-full overflow-x-auto rounded-md bg-surface-raised px-3 py-2 font-mono text-[11px] leading-5">
          {parts.table}
        </pre>
      )}
