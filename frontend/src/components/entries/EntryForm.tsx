...
import { Alert, Button, DateTimeField, Field, Input, Select, cx } from '@/components/ui';
  ...
  return (
    <form onSubmit={handleSubmit} noValidate className="flex flex-col gap-4">
      ...
      <div
        className={cx(
          'flex gap-5',
          !isEditing ? 'flex-row items-start' : 'flex-col',
        )}
      >
      ...
      {!isEditing && (
        <PhotoExtract
          className="w-[min(24rem,42%)] shrink-0"
          isAvailable={isAiAvailable}
          onApply={(result) => {
            const { entry: drafted } = result;

            setValues((current) => ({
              ...current,
              foodName: drafted.foodName,
              quantity: String(drafted.quantity),
              unit: drafted.unit,
              calories: String(drafted.calories),
              proteinGrams: String(drafted.proteinGrams),
              carbGrams: String(drafted.carbGrams),
              fatGrams: String(drafted.fatGrams),
              mealType: result.suggestedMealType ?? current.mealType,
            }));
            setMicronutrients(drafted.micronutrients);
            setFromPhoto(true);
          }}
        />
      )}
