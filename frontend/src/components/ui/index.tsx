...
const BUTTON_STYLES: Record<ButtonVariant, string> = {
  primary: 'bg-accent text-on-accent hover:bg-accent-hover font-bold shadow-[0_10px_22px_rgb(31_157_138/0.18)]',
  secondary: 'bg-surface text-foreground hover:bg-surface-raised border border-border-strong font-semibold',
  ghost: 'text-muted hover:text-foreground hover:bg-surface-raised font-semibold',
  danger: 'bg-transparent text-danger hover:bg-danger/10 border border-danger/30 font-semibold',
  outline: 'border border-accent bg-transparent text-accent hover:bg-accent-soft font-bold',
};
...
const CONTROL_STYLES =
  'w-full rounded-md border border-border-strong bg-surface px-3 py-2 text-sm text-foreground shadow-[inset_0_1px_0_rgb(15_23_42/0.02)] placeholder:text-subtle transition-colors focus:border-accent';
...
export function Card({
  ...
  return (
    <section
      className={cx(
        'rounded-md border border-border bg-surface p-5 shadow-[0_16px_36px_rgb(15_23_42/0.06)]',
        className,
      )}
    >
      {(title || action) && (
...
export function Skeleton({ className }: { className?: string }) {
  return <div className={cx('animate-pulse rounded-md bg-surface-raised', className)} />;
}

export function EmptyState({ title, description, action }: { title: string; description: string; action?: ReactNode }) {
  return (
    <div className="flex flex-col items-center gap-2 rounded-md border border-dashed border-border bg-surface/70 px-6 py-10 text-center">
