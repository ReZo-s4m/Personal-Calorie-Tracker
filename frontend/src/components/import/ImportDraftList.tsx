...
export function ImportDraftList({
  ...
  if (keysRef.current.length < rows.length) {
    keysRef.current = [
      ...keysRef.current,
      ...Array.from({ length: rows.length - keysRef.current.length }, () => crypto.randomUUID()),
    ];
  }
  ...
  return (
    ...
    <ul className="flex flex-col gap-3 lg:hidden">
      {rows.map((row, index) => (
        <li
          ...
          className="rounded-md border border-border bg-surface px-4 py-3 shadow-[0_1px_2px_rgb(17_17_19/0.04)]"
        >
          ...
          <input
            ...
            title={row.foodName}
            aria-label={`Food name, row ${index + 1}`}
            value={row.foodName}
    ...
    <div className="hidden overflow-x-auto rounded-md border border-border bg-surface shadow-[0_1px_2px_rgb(17_17_19/0.04)] lg:block">
      <table className="w-full min-w-[52rem] border-collapse text-sm">
        ...
        <tbody>
          {rows.map((row, index) => (
            <tr key={keysRef.current[index]} className="border-b border-border/70 last:border-0">
