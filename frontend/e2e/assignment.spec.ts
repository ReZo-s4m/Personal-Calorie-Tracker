...
async function expectToday(page: Page) {
  await expect(page.locator('aside').getByRole('link', { name: 'Overview' })).toBeVisible();
  await expect(page.getByRole('button', { name: 'Add Meal' }).first()).toBeVisible();
}

async function expectProfileName(page: Page, name: string) {
  await page.getByRole('button', { name: 'Open profile' }).click();
  await expect(page.getByRole('dialog', { name: 'Profile' }).getByText(name)).toBeVisible();
...
test.describe('Assignment end-to-end', () => {
  ...
  test('landing page: signed-out marketing home', async ({ page }) => {
    await page.goto('/');
    await expect(page.getByRole('heading', { name: /Log your meals/i })).toBeVisible();
    await expect(page.getByRole('link', { name: 'Sign up' }).first()).toBeVisible();
    await expect(page.getByRole('link', { name: 'Sign in' }).first()).toBeVisible();
    await page.getByRole('link', { name: 'Product' }).first().click();
    await expect(page.getByText('Meal logging', { exact: true })).toBeVisible();
    await expect(page.getByText('Photo extract', { exact: true })).toBeVisible();
  ...
  test('multi-user: sign up through the web app', async ({ page }) => {
    ...
    await expectProfileName(page, 'E2E User');
  ...
  test('meal entry: create breakfast/lunch/dinner/snacks with macros and micros', async ({ page }) => {
    ...
    await expect(page.locator('#meal-micro-add-nutrient')).toContainText('Vitamin C');
    await page.locator('#meal-micro-add-amount').fill('5');
    await page.getByRole('button', { name: 'Add', exact: true }).click();
    await expect(page.getByText('Vitamin C')).toBeVisible();

    await page.locator('#meal-micro-add-nutrient').click();
    await page.getByRole('option', { name: 'Iron', exact: true }).click();
    await expect(page.locator('#meal-micro-add-nutrient')).toContainText('Iron');
    await page.locator('#meal-micro-add-amount').fill('8');
    await page.getByRole('button', { name: 'Add', exact: true }).click();
  ...
  test('dashboard: recapture meals and goals from the database', async ({ page }) => {
    ...
    await nav.getByRole('link', { name: 'Add Meal' }).click();
    await expect(page.getByRole('heading', { name: 'Add Meal' })).toBeVisible();
    await nav.getByRole('link', { name: 'Target' }).click();
    await expect(page.getByRole('heading', { name: 'Target' })).toBeVisible();
    await nav.getByRole('link', { name: 'Weigh-in' }).click();
    await expect(page.getByRole('heading', { name: 'Weigh-in' })).toBeVisible();
  ...
  test('time-range listing: filter, search, edit, delete, paginate', async ({ page }) => {
    ...
    await expect(page.getByRole('dialog', { name: 'Edit meal' })).toBeVisible();
    ...
    await page.getByRole('dialog', { name: 'Delete this meal?' }).getByRole('button', { name: 'Delete meal' }).click();
  ...
  test('nutrition reports and graphs', async ({ page }) => {
    ...
    const goalCard = page.locator('section', { has: page.getByRole('heading', { name: 'Goal vs actual' }) });
    await expect(goalCard.locator('.recharts-radar')).toHaveCount(2);
    await expect(goalCard.locator('.recharts-polar-grid-angle line')).toHaveCount(5);
    expect(await goalCard.locator('.recharts-polar-grid-concentric-polygon').count()).toBeGreaterThanOrEqual(8);
    await expect(goalCard.locator('.recharts-polar-angle-axis-tick')).toHaveText([
      'Calories',
      'Protein',
      'Fat',
      'Carbs',
  ...
  test('bulk import via PDF: parse, review, commit', async ({ page }) => {
    ...
    const foodName = page.locator('table').getByLabel('Food name, row 1');
    await expect(foodName).toHaveValue('E2E imported khichdi');
    await foodName.click();
    await foodName.pressSequentially(' extra');
    await expect(foodName).toHaveValue('E2E imported khichdi extra');
    await expect(foodName).toBeFocused();
  ...
  test('AI extraction endpoint and chat surface exist', async ({ page, request }) => {
    ...
    if (available) {
      ...
      await expect(page.getByText(/Ask AI is switched off|Ask AI/)).toBeVisible();
    }
  });
