...
const micronutrientRules: ValidationChain[] = [
  body('micronutrients')
    .optional()
...
const coreEntryRules = (optional: boolean): ValidationChain[] => {
...
export const createEntryRules = [
  ...coreEntryRules(false),
  body('source').optional().isIn(['manual', 'image']),
];

const MAX_BATCH = 20;

export const createEntriesBatchRules = [
  body('entries')
  ...
  body('entries.*.micronutrients.*.nutrient')
    .trim()
    .notEmpty()
    .withMessage('Each micronutrient needs a name.')
    .matches(/^[a-z0-9_]+$/)
    .withMessage('Use lowercase letters, digits and underscores, for example "vitamin_c".')
    .isLength({ max: 60 }),
  body('entries.*.micronutrients.*.amount')
    .isFloat({ min: 0, max: 100_000 })
    .withMessage('Each micronutrient needs a non-negative amount.')
    .toFloat(),
  body('entries.*.micronutrients.*.unit').optional().trim().isLength({ min: 1, max: 12 }),
  body('source').optional().isIn(['manual', 'image']),
];

export const updateEntryRules = coreEntryRules(true);
...
export const entryIdRules = [param('id').trim().notEmpty().withMessage('Entry id is required.')];
