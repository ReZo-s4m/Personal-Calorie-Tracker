...
export class ReportPdfLogic implements IReportPdfLogic {
  constructor(
  ...
  async buildReportPdf(userId: string, query: ReportRangeQuery): Promise<ReportPdf> {
    const { from, to } = this.reportsLogic.resolveRange(query);
    ...
    const [profile, daily, weekly, macros, micronutrients, comparison] = await Promise.all([
      this.authLogic.getProfile(userId),
      this.reportsLogic.getDailyReport(userId, range),
      this.reportsLogic.getWeeklyReport(userId, { ...range, pageSize: MAX_WEEKS }),
      this.reportsLogic.getMacroBreakdown(userId, range),
      this.reportsLogic.getMicronutrientReport(userId, { ...range, pageSize: MAX_MICRONUTRIENTS }),
      this.reportsLogic.getTargetComparison(userId, range),
    ]);
    ...
    banner(
      ...
      `Generated ${formatDate(this.clock.now().toISOString().slice(0, 10))}`,
    );
    ...
    finish(doc, 'NutriAI — figures as recorded in your diary.');
...
type Comparison = TargetComparison;
type Daily = DailyReportRow[];
type Macros = MacroBreakdown;
type Micronutrients = MicronutrientRow[];
type Weekly = WeeklyReportRow[];
