...
import type { IRecommendLogic } from './IRecommendLogic.js';
...
export class RecommendLogic implements IRecommendLogic {
  constructor(
  ...
  async getRemainingNutrition(userId: string, date: string): Promise<RemainingNutrition> {
    const [goal, day] = await Promise.all([
      this.targetsLogic.getTargetForDate(userId, fromDateKey(date)),
      this.entriesLogic.listEntries(userId, {
  ...
  async recommendFoods(userId: string, date: string): Promise<MealRecommendation> {
    const remaining = await this.getRemainingNutrition(userId, date);
