...
# /reports/* orders the pair itself rather than rejecting it, unlike /entries.
echo -n "  a reversed range is ordered, not rejected: "
curl -s -o /dev/null -w "%{http_code}\n" "$BASE/reports/pdf?from=$TODAY&to=$(day 5)" -H "$AUTH"
