...
describe('API edge cases', () => {
  ...
  it('rejects a report window longer than a year and still accepts a swapped pair', async () => {
    ...
    // Relative to today, so the window stays over a year as the calendar moves.
    const wide = await request(base, `/reports/daily?from=${daysAgoKey(400)}&to=${today}`, { token });
    assert.equal(wide.status, 400);

    const swapped = await request(
      base,
      `/reports/daily?from=${today}&to=${daysAgoKey(5)}&pageSize=5`,
      { token },
    );
