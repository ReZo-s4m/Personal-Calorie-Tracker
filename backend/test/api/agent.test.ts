...
import { attachmentsLogic, chatTools, pendingLogic } from '../../src/container.js';
import { createPending } from '../../src/modules/chat/pending/pending.factory.js';


import { daysAgoKey, request, signup, startApi, todayKey } from '../helpers.js';

// Drive the same instances the HTTP layer uses, straight from the composition root.
const runTool = chatTools.run.bind(chatTools);
const applyPending = pendingLogic.applyPending.bind(pendingLogic);
const applyAttachPending = attachmentsLogic.applyAttachPending.bind(attachmentsLogic);
